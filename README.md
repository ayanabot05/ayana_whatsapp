...
AYANA's Routing Strategy
First contact is always a template. When a parent is added, both the parent and the child are welcomed with the approved ayana_opener template (send_welcome_for_new_parent → send_opener_welcome). A brand-new (cold) number has no open session, so a template is the only thing that will deliver. The child's signup welcome is intentionally gated until the first parent is added.

Templates carry quick-reply buttons, so a parent can tap Good / Okay / Not-well (or Taken / Skipped) even with the session closed — no dead air while waiting for a session.

Daily check-ins: session closed → approved template (with buttons); session open → free interactive quick-reply buttons as a cost optimisation with richer rotating content.

Out-of-session button taps return {id, title}. AYANA maps them to the right intent by structured id, or — when a template button has no structured id — by matching the localised title the parent tapped (Good / బాగున్నా / अच्छा, Taken / వేసుకున్నా / ले लिया).
...
WhatsApp Templates
6 Meta-Approved Template Families (× en/te/hi = 18)
...
Database Schema
Granular Schedule Tables (v2)
The scheduler now reads from granular, user‑configured tables instead of a monolithic JSON blob:

Table	Purpose
parent_checkins	Daily check‑ins (morning_wish, breakfast, lunch, dinner, afternoon_checkin, goodnight, love_note) with user‑set times
parent_health_reminders	Health reminders (water, bp_check, sugar_check, health_check)
parent_routines	Routines (tea_check, walk_check)
medicines	Individual medicines with reminder_times as a JSON array (e.g., ["09:00", "21:00"])
The scheduler (via get_parent_schedule()) queries these tables every minute and sends messages at exactly the user‑configured times, in the parent's local timezone. This eliminates flooding (cron respects timezone), ensures no unsolicited medicine reminders (only loops over medicines that exist), and makes the schedule fully visible and editable per category.
...
schedules	Legacy table (kept for backward compatibility; no longer used by the scheduler)
message_logs	Outbound message history with delivery status
parent_replies	Inbound replies from parents
emergency_events	Distress alerts (never purged)
distress_logs	AI distress classification history
monthly_reports	Aggregated monthly summaries (never purged)
moments	Child→parent photo/text messages
payment_state	Current plan and billing status
phone_otps	OTP codes for verification
jwt_blacklist	Revoked tokens
scheduler_locks	Distributed scheduler locks
...
Known Limitations
Meta template approval required — the 6 template families must be approved in Meta Business Manager before out-of-session messages send.

Payments are a one-time charge per plan change, not a subscription — payments.py uses Stripe Checkout in mode="payment". No recurring subscription, proration, or billing-cycle adjustment. This is the true launch blocker for recurring SaaS; needs Stripe Subscriptions (price ID per plan, customer object, invoice.paid / customer.subscription.updated webhooks).

Channel dependency — one WhatsApp number / one Meta account. If Meta flags it, every customer goes dark at once. Add a backup number + quality-rating monitoring before scaling past the first ~50 paying users.

Distress ML (Layer 2) is off by default and beta — needs SARVAM_API_KEY, framed as assistive, never as emergency detection.

Performance — cross-region Railway↔Supabase latency; a region move (e.g. Singapore) and collapsing N+1 queries are pending.

server.py is a large monolith — a planned refactor into per-domain routers (not yet done; carries regression risk, so scheduled separately).

Frontend migration pending — the Dashboard.jsx ParentDialog still uses the old /schedules endpoint and parents.medicine_list. The backend is ready with granular tables (parent_checkins, parent_health_reminders, parent_routines, medicines), but the frontend must be updated to read/write from these new endpoints. See the frontend migration notes in the code.
