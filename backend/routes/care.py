...
async def care_status(parent_id: UUID,user=Depends(get_current_user)):
    ...
    result = {'timezone':parent['timezone'],'next_at':None,'next_label':None,'welcome':dict(welcome) if welcome else None,'last_delivery':last['delivery_status'] if last else None}
