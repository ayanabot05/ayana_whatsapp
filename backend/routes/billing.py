...
async def webhook(request: Request):
    ...
    sub = await get_pool().fetchrow(
        'SELECT * FROM billing_subscriptions WHERE gateway_subscription_id=$1', gateway_sub_id
    )
    if not sub:
        return {'ok': True, 'ignored': True}
    ...
    if event_type == 'subscription.charged':
        # Razorpay auto-charged the customer for a new period.
        payment_entity = payload_data.get('payment', {}).get('entity', {})
        period_start = now
        # current_end for monthly = now + 1 month; for yearly = now + 1 year
        from services.billing_access import add_period
        period_end = add_period(now, sub['billing'])

        async with get_pool().acquire() as conn, conn.transaction():
            await conn.execute(
                'SELECT pg_advisory_xact_lock(hashtextextended($1,0))', 'billing:' + str(sub['user_id'])
            )
            # Create a new access_grant for this period
            grant = await conn.fetchrow(
                """INSERT INTO access_grants (user_id, order_id, plan, starts_at, ends_at)
                   SELECT $1, billing_orders.id, $3, $4, $5
                   FROM billing_orders WHERE user_id=$1
                   LIMIT 1
                   ON CONFLICT DO NOTHING
                   RETURNING id""",
                sub['user_id'], sub['billing_plan_id'], sub['plan'], period_start, period_end,
            ) if False else None  # access_grants requires an order_id FK — use payment_state instead

            # Update payment_state for access gating
            await conn.execute(
                """INSERT INTO payment_state (user_id, status, plan, billing, billing_managed)
                   VALUES ($1, 'active', $2, $3, true)
                   ON CONFLICT (user_id) DO UPDATE
                       SET status='active', plan=EXCLUDED.plan, billing=EXCLUDED.billing,
                           billing_managed=true, updated_at=now()""",
                sub['user_id'], sub['plan'], sub['billing'],
            )
            await conn.execute(
                """UPDATE billing_subscriptions
                   SET status='active', current_period_start=$2, current_period_end=$3, updated_at=now()
                   WHERE id=$1""",
                sub['id'], period_start, period_end,
            )

    elif event_type == 'subscription.halted':
        # Payment failed after all retries — suspend access at period end
        await get_pool().execute(
            "UPDATE billing_subscriptions SET status='halted', updated_at=now() WHERE id=$1", sub['id']
        )
        await get_pool().execute(
            "UPDATE payment_state SET status='unpaid', updated_at=now() WHERE user_id=$1", sub['user_id']
        )

    elif event_type == 'subscription.cancelled':
        await get_pool().execute(
            "UPDATE billing_subscriptions SET status='cancelled', cancelled_at=now(), updated_at=now() WHERE id=$1",
            sub['id'],
        )
        ...
        await get_pool().execute(
            "UPDATE billing_subscriptions SET status=$2, updated_at=now() WHERE id=$1",
            sub['id'], event_type.split('.')[1],
        )

    elif event_type == 'subscription.authenticated':
        await get_pool().execute(
            "UPDATE billing_subscriptions SET status='authenticated', updated_at=now() WHERE id=$1", sub['id']
        )
