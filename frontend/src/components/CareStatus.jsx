...
export const CareStatus = ({ parentId }) => {
  ...
  return <div className="my-3 rounded-xl bg-ayana-bg border border-ayana-line p-3 text-xs space-y-1" data-testid={`care-status-${parentId}`}>
    ...
    {data.welcome && <p className="text-ayana-secondary" data-testid={`care-welcome-${parentId}`}>Welcome: {data.welcome.status === 'accepted' ? 'submitted, awaiting delivery confirmation' : data.welcome.status}</p>}
  </div>;
};