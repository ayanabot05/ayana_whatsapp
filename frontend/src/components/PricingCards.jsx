  ...
  const getPriceDisplay = (price) => {
    ...
    if (billing === "month") {
      return {
        ...
        label: "/mo",
        sub: 'Billed monthly after you authorize a paid subscription.',
        crossed: null,
        savings: null,
      };
    }
    // Work in minor units; display the equivalent, never imply monthly charges.
    const monthCents = Math.round(price.month * 100);
    const yearCents = Math.round(price.year * 100);
    const equivalent = Math.round(yearCents / 12) / 100;
    const saved = (monthCents * 12 - yearCents) / 100;
    const twoFree = yearCents === monthCents * 10;
    return {
      big: `${cur.symbol}${fmtNum(equivalent)}`,
      label: '/mo equivalent',
      crossed: `${cur.symbol}${fmtNum(price.month)}`,
      sub: `${cur.symbol}${fmtNum(price.year)} billed once per year for 12 months of access.`,
      savings: saved > 0 ? `Save ${cur.symbol}${fmtNum(saved)} annually${twoFree ? ' · 2 months free' : ''}` : null,
    };
  };
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-6">
        <div className="inline-flex rounded-full border border-ayana-line bg-white p-1" data-testid="billing-toggle">
          {[["month", "Monthly"], ["year", "Annual"]].map(([k, label]) => (
            ...
            <div key={p.id} data-testid={`plan-${p.id}`}
              ...
              <div className="mt-5">
                <div className="flex items-end gap-2">
                  <span className="font-display text-4xl font-black text-ayana-text" data-testid={`plan-price-${p.id}`}>{display.big}</span>
                  {display.crossed && (
                ...
                {display.sub && (
                  <p className="text-xs text-ayana-muted mt-1" data-testid={`plan-billing-explanation-${p.id}`}>{display.sub}</p>
                )}
                {display.savings && (
                  <p className="text-xs font-semibold text-emerald-600 mt-1" data-testid={`plan-savings-${p.id}`}>{display.savings}</p>
                )}
              </div>
